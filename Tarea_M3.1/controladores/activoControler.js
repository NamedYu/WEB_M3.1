    ...
    if (index !== -1) {
        ...
        if(listaActivos.findIndex(activo => activo.id == id)!=-1){
            ...
            const activoActualizado = {
                id:id || listaActivos[index].id,
                numeroSerie: numeroSerie || listaActivos[index].numeroSerie,
                numeroInventarioUABC: numeroInventarioUABC || listaActivos[index].numeroInventarioUABC,
                tipo: tipo || listaActivos[index].tipo,
                descripcion: descripcion || listaActivos[index].descripcion,
                ubicacion: ubicacion || listaActivos[index].ubicacion,
                responsable: responsable || listaActivos[index].responsable,
                imagen: imagen || listaActivos[index].imagen
            };
...
const agregarNuevoActivo = (req, res) => {
    ...
    if(listaActivos.findIndex(activo => activo.id == id)!=-1){
        ...
        res.status(201).json(listaActivos);
    }
};
